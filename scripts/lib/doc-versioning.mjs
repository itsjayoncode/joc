 ...
 * creating `apps/docs/doc-versions/<id>.json` with `archive: { major, minor, patch }`,
 * wiring VitePress versions meta / the version switcher, and bootstrapping archives on first
 * release (`pnpm docs:archive -- --package <id> --bootstrap`).
 *
...
/** @typedef {{ readonly major: boolean; readonly minor: boolean; readonly patch: boolean }} DocArchiveFlags */

/** Keep in sync with scripts/doc-versioning-policy.ts */
export const DEFAULT_DOC_ARCHIVE_FLAGS = {
  major: true,
  minor: true,
  patch: false,
};
...
/**
 * Resolve archive flags from manifest `archive` object or legacy `archivePolicy` string.
 * Keep in sync with scripts/doc-versioning-policy.ts
 *
 * @param {Partial<DocArchiveFlags> | "minor-major" | "minor" | "major" | null | undefined} archive
 * @returns {DocArchiveFlags}
 */
export function resolveDocArchiveFlags(archive) {
...
/**
 * @param {string} _currentVersion
 * @param {string | null} bumpType
 * @param {Partial<DocArchiveFlags> | "minor-major" | "minor" | "major" | null | undefined} archive
 */
export function shouldArchiveForBumpType(_currentVersion, bumpType, archive) {
  // Keep in sync with scripts/doc-versioning-policy.ts
...
/** @param {{ archive?: Partial<DocArchiveFlags>; archivePolicy?: string }} manifest */
export function readManifestArchiveConfig(manifest) {
  return manifest.archive ?? manifest.archivePolicy;
}
...
export function syncPackageVersionsMeta(pkg) {
  ...
  const archive = resolveDocArchiveFlags(readManifestArchiveConfig(manifest));
