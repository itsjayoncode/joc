/** Copy text with Clipboard API + execCommand fallback. */
...
const SHARE_HASH_PREFIX = "lab=";

export function encodeLabShareHash(payload: unknown): string {
...
export function decodeLabShareHash(hash: string): unknown {
...
export function buildLabShareUrl(payload: unknown): string {
  const url = new URL(window.location.href);
  url.hash = encodeLabShareHash(payload);
  return url.toString();
}
