...
Phase 3.10 adds `/plugins` to the Object Diff Playground and implements a real plugin runtime in `@jayoncode/object-diff`.

## Package changes

- Extended `BrowserLifecyclePlugin` with lifecycle hooks and metadata
- Added `PluginRuntime` under `packages/object-diff/src/plugins/`
