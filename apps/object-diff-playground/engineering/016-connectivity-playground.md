...
ConnectivityPage
  -> useConnectivityPlayground()
    -> createConnectivityPlaygroundSession() in lib/object-diff.ts
...
Phase `3.6` required the Connectivity module in `@jayoncode/object-diff`:

- `packages/object-diff/src/modules/connectivity/`
- public events: `connection:online`, `connection:offline`, `connection:reconnect`
- snapshot field: `connectivity`

## Tests

- `src/lib/object-diff.test.ts` covers connectivity integration helpers and event-log formatting
