...
`@jayoncode/form-intelligent` uses a **domain engine** layout inside one published package. Optional engines are exposed via subpath exports. Framework adapters (`-react`, `-vue`, `-angular`) ship as separate packages — see [Ecosystem architecture](./001-ecosystem-architecture.md) and [Modular packages](./003-modular-packages.md).
