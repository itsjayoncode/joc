# Form Intelligent — Modular Package Architecture

## Goal

One published npm package (`@jayoncode/form-intelligent`) organized by **domain engines**, with optional engines exposed as **subpath exports**. Heavy engines load only when imported or enabled via config / `form.use(module())`.

Framework adapters and third-party integrations stay in **separate packages**.

This mirrors **Browser Lifecycle**: thin session core → module registry → opt-in modules → framework adapters. Form Intelligent applies the same pattern inside a single published package instead of fragmenting into many npm installs.
...
| ------------------------------------- | -------------------------------------------- | ---------------------- |
| `@jayoncode/form-intelligent`         | All engines (core + optional subpaths)       | **Target**             |
| `@jayoncode/form-intelligent-react`   | `useForm()`, React bindings                  | **Shipped**            |
| `@jayoncode/form-intelligent-zod`     | Zod schema adapter                           | **Shipped**            |
| `@jayoncode/form-intelligent-yup`     | Yup schema adapter                           | **Shipped**            |
| `@jayoncode/form-intelligent-valibot` | Valibot schema adapter                       | **Shipped**            |
...
```
@jayoncode/form-intelligent              → core barrel (engines: core, state, field, validation, submission, plugin)
@jayoncode/form-intelligent/validation
@jayoncode/form-intelligent/format
@jayoncode/form-intelligent/rules        → alias of workflow rules surface
@jayoncode/form-intelligent/workflow
@jayoncode/form-intelligent/draft        → NEW (split from workflow)
@jayoncode/form-intelligent/wizard       → NEW (split from workflow)
@jayoncode/form-intelligent/submission
@jayoncode/form-intelligent/plugins
@jayoncode/form-intelligent/analytics
@jayoncode/form-intelligent/offline
@jayoncode/form-intelligent/history
@jayoncode/form-intelligent/devtools       → inspector plugin + global registry (opt-in)
```
...
import { createOfflineModule } from "@jayoncode/form-intelligent/offline";
import { createAnalyticsModule } from "@jayoncode/form-intelligent/analytics";
...
```ts
import { createForm } from "@jayoncode/form-intelligent";
import { createBrowserSessionModule } from "@jayoncode/form-intelligent/plugins";
...
- [x] CI hook via `pnpm --filter @jayoncode/form-intelligent check:size`

Fixtures:

| Fixture          | Import surface                      | Budget (gzip) |
| ---------------- | ----------------------------------- | ------------- |
| `core-login`     | `createForm` + validators from main | 24 KB         |
...
- [x] `@jayoncode/form-intelligent-zod` — `zodAdapter()` → `SchemaAdapter`
- [x] `@jayoncode/form-intelligent-yup` — `yupAdapter()` → `SchemaAdapter`
- [x] `@jayoncode/form-intelligent-valibot` — `valibotAdapter()` → `SchemaAdapter`
- [x] `@jayoncode/form-intelligent-ajv` — `ajvAdapter()` → `SchemaAdapter`
- [x] `@jayoncode/form-intelligent-vue` — `useForm`, `provideForm`, `useField`
- [x] `@jayoncode/form-intelligent-angular` — `FormService`, signals, `fiForm` / `fiField` directives
...
```
@jayoncode/form-intelligent          (single published package — all engines)
        ↑
...
- Adapters depend on `@jayoncode/form-intelligent`
