...
Tree-shakeable imports for `@jayoncode/form-intelligent`. Prefer the **narrowest** entry that exports what you need.
...
| Need                                                                | Import from                              |
| ------------------------------------------------------------------- | ---------------------------------------- |
| `createForm`, `when`, validators, most instance APIs                | `@jayoncode/form-intelligent` (main)     |
| Phone/currency **masks**, `formatSlug`, `trim`, `composeFormatters` | `@jayoncode/form-intelligent/format`     |
| DevTools inspector                                                  | `@jayoncode/form-intelligent/devtools`   |
| Browser-lifecycle / keyboard plugins                                | `@jayoncode/form-intelligent/plugins`    |
| Error helpers (`toNormalizedErrors`, …)                             | `@jayoncode/form-intelligent/validation` |
...
`trim` stays `trim`. See [Formatters](/packages/form-intelligent/modules/formatters) and [Validation](/packages/form-intelligent/modules/validation#basics--built-in-validators).

## Subpath map

| Import                        | Use for                                                                                                                                              | Also on main?                                    |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `@jayoncode/form-intelligent` | `createForm`, `when`, validators, `calculate`, `dependencies`, transform/presentation/middleware helpers, `createFormController`, `computeFieldAria` | —                                                |
...
```ts
import { createForm, when, required, email } from "@jayoncode/form-intelligent";
```
...
- [Formatters](/packages/form-intelligent/modules/formatters) — format vs transform
- [Integrations](/packages/form-intelligent/modules/integrations) — plugins + DevTools
- [Performance](/packages/form-intelligent/modules/performance) — budgets and tree-shaking
